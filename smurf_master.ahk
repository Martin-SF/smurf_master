...
/* if not A_IsAdmin
{
	Run  *RunAs "%A_ScriptFullPath%" ;required for blockinput and close steam (or maybe not)
	Exitapp
} 
*/

PWpw := "5ZGcTcHQszjKMv764FGkPeYpCbwnppfURpJPPahN9zjsn7zjAE27SxaFpxmgfjF6" ;for this script
ver = v0.2.4-alpha ;SMURF MASTER by Peter Holz (bananacry) Alpha TESTING
...
{ ;for secureproducts
	...
	programname = smurf Master
	programininame = smurf.ini
	programlogname = smurf.log

	#include, SecureProducts.ahk ;includes license check and important decryption functions
}
return
...
steamrunerror(mode = "") {
	...
	if (mode = "close") {
		...
		Gui, Add, Picture, x22 y49 w380 h540 , %A_Temp%\admin.png
...
Check:
	...
	if winactive("ahk_exe steam.exe ") = 0
		WinActivate, ahk_exe steam.exe
return
